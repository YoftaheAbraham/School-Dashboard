...
const SubjectsDisplayCard = ({ Name, Year, Grade }: { Name: string, Year: string; Grade: string[] }) => {
    return (
            ...
            <div className="max-w-full overflow-x-auto no-scrollbar flex justify-between dark:scrollbar-thumb-dark-3 dark:scrollbar-track-gray-dark">
                <div>
                    ...
                    >
                        ...
                        {Name} | {Year}</h1>
                    <p className='text-sm mb-2'>Teacher: <span className='font-semibold'>John Doe</span></p>
                    <p className='flex items-center gap-2'>
                        Grades:
                        <div className='flex gap-2 flex-wrap'>
                            {Grade.map((item, index) => {
                                return <div key={index} className='flex justify-center items-center border w-8 h-8 text-sm text-center rounded border-stroke'>{item}</div>
                            })}
                        </div>
                    </p>
...
export default SubjectsDisplayCard